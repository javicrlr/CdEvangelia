<script type="text/javascript">
	function columnsshowmaina(id_select_a){
		...
		position_e = document.getElementById("position_e" + value_id);
		var text_b = '';
		position_b.innerHTML = text_b;
		text_b += '<select class="selectdismode" onchange="columnsshowmainb(this.value)">';
	...
	function columnsshowmainc(id_select_c){
		var text_d = '';
		position_d.innerHTML = text_d;
		text_d += '<select class="selectdismode" onchange="columnsshowmaind(this.value)">';
	...
	function columnsshowmaind(id_select_d){
		var text_e = '';
		position_e.innerHTML = text_e;
		text_e += '<select class="selectdismode" onchange="columnsshowmaine(this.value)">';
		text_e += '<option value=""><?php echo JText::_('TPL_CDEVANGELIA_CDSELECTCOLUMNS_LABEL') ?></option>';
		column_d = id_select_d.slice(10);
		column_select_d = parseInt(column_d);
		var diferencia_select_d = 12 - (column_select_d + column_select_c + column_select_b + column_select_a); 
		for (i=0 ; i <= diferencia_select_d; i++){
			text_e += '<option value="' + id_select_d + '-e' + i +'">' + i + ' <?php echo JText::_('TPL_CDEVANGELIA_CDSELECTCOLUMNSNAME_LABEL') ?></option>';
		}
		text_e +='</select>';
		position_e.innerHTML = text_e;
		document.getElementById('id_select' + value_id).value = '';
	...
	function columnsshowmaine(id_select_e){
		column_e = id_select_e.slice(13); 
		column_select_e = parseInt(column_e);
		document.getElementById('id_select' + value_id).value = '';
		document.getElementById('id_select' + value_id).value = id_select_e;
	}
</script>
<?php
...
class JFormFieldDismodulemain extends JFormField {
	protected $type = 'Dismodulemain';
	public function getInput() {
		return '<input id="id_select'.$this->id.'" type="text" name="'.$this->name.'"  value="'.$this->value.'">'.
		'<select id="'.$this->id.'" class="selectdismode" onchange="columnsshowmaina(this)">'.
